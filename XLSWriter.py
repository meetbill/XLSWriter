...
class XLSWriter(object):
    ...
    def add_big_head(self, bmp_show='', x='', y='', length='',
                     title_name='ceshi', sheet_name='sheet'):
        """
        添加 head
        Args:
            bmp_show:是否显示 logo
    ...
    def create_sheet(self, sheet_name='sheet'):
        ...
        self.sheets[sheet_name]['sheet'] = self.wbk.add_sheet('%s%s' % (
            sheet_name, sheet_index if sheet_index else ''), cell_overwrite_ok=True)
...
if __name__ == '__main__':
    ...
    # 创建"基本信息" sheet，并且使用 4 列单元格，首行合并单元格，并新增logo
    xlswriter.add_big_head(True, 0, 0, 4, title_name=u"测试", sheet_name=u"基本信息")
    # 创建"基本信息" sheet，并且使用 4 列单元格，首行合并单元格，不使用 logo
    xlswriter.add_big_head(False, 0, 0, 4, title_name=u"测试", sheet_name=u"没logo")
    xlswriter.add_header(u"信息登记表", 4, sheet_name=u"基本信息")
    xlswriter.add_remark(
        u"信息登记表jkfdadaddddddddd\nddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd",
        4,
        sheet_name=u"基本信息")
    xlswriter.setcol_width([20, 20, 20, 20], sheet_name=u'基本信息')

    xlswriter.writerow(['姓名', '年龄', '电话', 'QQ'],
                       sheet_name=u'基本信息', border=True, pattern_n=22)
    xlswriter.writerow(['张三', '30', '12345678910', '123456789'],
                       sheet_name=u'基本信息', border=True)
    xlswriter.writerow(['王五', '30', '13512345678', '123456789'],
                       sheet_name=u'基本信息', border=True, pattern_n=2)

    xlswriter.writerow(['检测项', '检测命令', '值', '基准', '状态'], sheet_name=u'服务器器状态')
    xlswriter.writerow(["磁盘空间",
                        "df -lP | grep -e '/$' | awk '{print $5}'",
                        "20%",
                        "%85",
                        "OK"],
                       sheet_name=u'服务器器状态')
